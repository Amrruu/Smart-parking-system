...
void LCD_sendchar(s_LCD_Config_t *LCD, unsigned char data, e_LCDCharType_t e_LCDCharType)
{
	...
	if (LCD->e_OperMode < EIGHTBIT_1LINE_7DOT) // 4 bits
	{
		...
		MCAL_GPIO_WriteGroup(
				LCD->s_LCDDataPins.GPIOX, ((data & 0xF0) >> 4) * LCD->s_LCDDataPins.DataPinStart, 0x0F * LCD->s_LCDDataPins.DataPinStart);
		...
		MCAL_GPIO_WriteGroup(LCD->s_LCDDataPins.GPIOX, (data & 0x0F) * LCD->s_LCDDataPins.DataPinStart, 0x0F * LCD->s_LCDDataPins.DataPinStart);
	...
	{ // 8bit mode
		...
		MCAL_GPIO_WriteGroup(LCD->s_LCDDataPins.GPIOX, (data & 0xFF) * LCD->s_LCDDataPins.DataPinStart, 0xFF * LCD->s_LCDDataPins.DataPinStart);
		if (e_LCDCharType == COMMAND)
			// BITCLEAR(*(LCD->s_LCDCtrlPins.p_PortReg),LCD->s_LCDCtrlPins.rspin);
			MCAL_GPIO_WritePin(LCD->s_LCDCtrlPins.GPIOX, LCD->s_LCDCtrlPins.rspin, GPIO_PIN_RESET);
		else
